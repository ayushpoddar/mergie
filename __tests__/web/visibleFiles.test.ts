...
import { visibleFiles, searchFiles, type VisibilityToggles } from "@/web/lib/visibleFiles.ts";
...
describe("visibleFiles", () => {
  test("returns all files unchanged with no toggles", () => {
...
describe("searchFiles", () => {
  test("returns all files in original order with an empty query", () => {
    expect(searchFiles(FILES, "").map((f) => f.newPath)).toEqual([
      "src/a.ts", "src/b.ts", "package-lock.json",
    ]);
  });

  test("fuzzy-filters the file list to matches", () => {
    expect(searchFiles(FILES, "lock").map((f) => f.newPath)).toEqual(["package-lock.json"]);
  });

  test("does not mutate the input array", () => {
    const input = [...FILES];
    searchFiles(input, "src");
    expect(input.map((f) => f.newPath)).toEqual(["src/a.ts", "src/b.ts", "package-lock.json"]);
  });
});

...
const MY_PR: MyPrSummary = {
  ...
  author: "ayush", isDraft: false, updatedAtIso: "2026-07-15T00:00:00Z", relationships: ["authored"],
};

function fakeSearch(): GhSearchService {
  return { listMyPrs: async () => [MY_PR] };
}
...
function fakeRegistry(): PrRegistry {
  ...
  return {
    async loadPr(url) {
      const pr: LoadedPr = { id: `pr-${loaded.length + 1}`, url, owner: "o", repo: "r", number: 1, title: "T", body: "", baseRef: "main", headRef: "feature" };
      loaded.push(pr);
      return pr;
    },
    listPrs: () => loaded,
